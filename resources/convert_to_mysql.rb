...
filename = "formatted_simplified_kanjidic2.xml"
...
database = DBI.connect("DBI:Mysql:kanjidict2:localhost", "root", Base64::decode64('JDRuZDAxY2gx') )

row = database.select_one("SELECT VERSION()")
puts "Server version: " + row[0]

# Clearing out all tables
	db.do('DROP TABLE IF EXISTS kanji;')
	db.do('DROP TABLE IF EXISTS kanji_codepoint;')
	db.do('DROP TABLE IF EXISTS kanji_radical;')
	db.do('DROP TABLE IF EXISTS kanji_misc;')
	db.do('DROP TABLE IF EXISTS kanji_dict_ref;')
	db.do('DROP TABLE IF EXISTS kanji_query_code;')
	db.do('DROP TABLE IF EXISTS kanji_reading;')
	db.do('DROP TABLE IF EXISTS kanji_meaning;')
